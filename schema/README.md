...
A simple example of a Boutiques application descriptor:
```javascript
{
  ...
  // The actual command line run will be generated by substituting given input values into this string, using the "command-line-key" property of the input. Notice we can chain multiple commands easily
  ...
  "inputs" : [{
    ...
    "id" : "num_input", // A unique id for this input param, so platforms can identify it
    "name" : "A number input",
    "type" : "Number",
    ...
    // Can also specify a list of ids that are not permitted to be used (added to command-line) when this parameter is active (given)
  ...
  // Parameter groups represent semantically related sets of input parameters
  // They can be used to specify inter-parameter disabling/requirement, and to help automatic UI generation on platforms
  "groups" : [{
    "id" : "an_example_group",
    ...
    // When true, at least one of the group members must be specified in the input
