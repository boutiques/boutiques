...
def add_subparser_create(subparsers):
    ...
    parser_create.add_argument("--camel-case", action="store_true",
...
def add_subparser_data(subparsers):
    ...
    data_subparsers = parser_data.add_subparsers(help="Manage execution data records. Inspect: displays "
    ...
    parser_data_publish.add_argument("-a", "--author", action="store",
                                     help="Set the author name for the data set "
                                     "publication. Defaults to anonymous.")
    parser_data_publish.add_argument("-f", "--file", action="store",
                                     help="Filename of record to publish alone as a "
                                     "data set.")
    parser_data_publish.add_argument("-i", "--individually", action="store_true",
    ...
    parser_data_publish.add_argument("--sandbox", action="store_true",
                                     help="publish to Zenodo's sandbox instead of "
                                     "production server. Recommended for tests.")
    parser_data_publish.add_argument("--zenodo-token", action="store",
    ...
    parser_data_publish.add_argument("--nexus-token", action="store",
...
def add_subparser_deprecate(subparsers):
    parser_deprecate = subparsers.add_parser("deprecate", description="Deprecates a published descriptor by"
    ...
    parser_deprecate.add_argument("--by", action="store", help="Zenodo id (e.g., "
                                  "zenodo-1234) of a  descriptor that will supersede "
                                  "the deprecated one.")
    parser_deprecate.add_argument("--zenodo-token", action="store",
                                  help="Zenodo API token to use for authentication. "
...
def add_subparser_evaluate(subparsers):
    parser_evaluate = subparsers.add_parser("evaluate", description="Evaluates parameter values for a"
                                            " descriptor and invocation")
    parser_evaluate.set_defaults(function='evaluate')
    parser_evaluate.add_argument("descriptor", action="store",
    ...
    parser_evaluate.add_argument("query", action="store", nargs="*",
                                 ...
                                 "space.")
    parser_evaluate.add_argument("--sandbox", action="store_true",
                                 help="Get descriptor from Zenodo's sandbox instead of "
                                 "production server.")


def add_subparser_example(subparsers):
    parser_example = subparsers.add_parser("example", description="Generates example invocation from a"
                                           " valid descriptor")
    parser_example.set_defaults(function='example')
    parser_example.add_argument("descriptor", action="store",
    ...
    parser_example.add_argument("--sandbox", action="store_true",
                                help="Get descriptor from Zenodo's sandbox instead of "
                                "production server.")
...
def add_subparser_execute(subparsers):
    ...
    exec_subparsers = parser_exec.add_subparsers(help="Mode of operation to use. Launch: takes a "
    ...
    parser_exec_launch = exec_subparsers.add_parser("launch", description="Launches an invocation.")
    parser_exec_launch.set_defaults(mode='launch')
    parser_exec_launch.add_argument("descriptor", action="store",
                        help="The Boutiques descriptor as a JSON file, "
                        "JSON string or Zenodo ID (prefixed by 'zenodo.').")
    parser_exec_launch.add_argument("invocation", action="store",
                        help="Input JSON complying to invocation.")
    parser_exec_launch.add_argument("-v", "--volumes", action="append", type=str,
    ...
    parser_exec_launch.add_argument("-x", "--debug", action="store_true",
                        ...
                        "messages.")
    parser_exec_launch.add_argument("-u", "--user", action="store_true",
                        help="Runs the container as local user ({0})"
                        " instead of root.".format(os.getenv("USER")))
    parser_exec_launch.add_argument("-s", "--stream", action="store_true",
                        help="Streams stdout and stderr in real time "
                        "during execution.")
    parser_exec_launch.add_argument("--imagepath", action="store",
                        help="Path to Singularity image. "
                        "If not specified, will use current directory.")
    parser_exec_launch.add_argument("--skip-data-collection", action="store_true",
                        help="Skips execution data collection and saving"
                        "to cache.")
    parser_exec_launch.add_argument("--provenance", action="store", type=json.loads,
                        help="Append JSON object to the generated record.")
    parser_exec_launch.add_argument("--no-container", action="store_true",
                        help="Launch invocation on the host computer, with "
                        "no container. If 'container-image' appears in the "
                        "descriptor, it is ignored.")
    parser_exec_launch.add_argument("--sandbox", action="store_true",
                        help="Get descriptor from Zenodo's sandbox instead of "
                        "production server.")
    ...
    parser_exec_prepare = exec_subparsers.add_parser("prepare", description="Pulls the container image for a given "
                            "descriptor")
    parser_exec_prepare.set_defaults(mode='prepare')
    parser_exec_prepare.add_argument("descriptor", action="store",
                        help="The Boutiques descriptor as a JSON file, "
                        "JSON string or Zenodo ID (prefixed by 'zenodo.').")
    parser_exec_prepare.add_argument("-x", "--debug", action="store_true",
                        ...
                        "execution, and prints additional debug "
                        "messages.")
    parser_exec_prepare.add_argument("-s", "--stream", action="store_true",
                        help="Streams stdout and stderr in real time "
                        "during execution.")
    parser_exec_prepare.add_argument("--imagepath", action="store",
                        help="Path to Singularity image. "
                        "If not specified, will use current directory.")
    parser_exec_prepare.add_argument("--sandbox", action="store_true",
                        help="Get descriptor from Zenodo's sandbox instead of "
                        "production server.")

    parser_exec_simulate = exec_subparsers.add_parser("simulate", description="Simulates an invocation.")
    parser_exec_simulate.set_defaults(mode='simulate')
    parser_exec_simulate.add_argument("descriptor", action="store",
                        help="The Boutiques descriptor as a JSON file, "
                        "JSON string or Zenodo ID (prefixed by 'zenodo.').")
    parser_exec_simulate.add_argument("-i", "--input", action="store",
                        help="Input JSON complying to invocation.")
    parser_exec_simulate.add_argument("-j", "--json", action="store_true",
                        help="Flag to generate invocation in JSON format.")
    parser_exec_simulate.add_argument("-c", "--complete", action="store_true",
                        help="Include optional parameters.")
    parser_exec_simulate.add_argument("--sandbox", action="store_true",
                        help="Get descriptor from Zenodo's sandbox instead of "
                        "production server.")


def add_subparser_export(subparsers):
    parser_export = subparsers.add_parser("export", description="Export Boutiques descriptor to"
                                          " other formats.")
    ...
    parser_export.add_argument("--identifier", help="Identifier to use in"
                               "CARMIN export.")
    parser_export.add_argument("--sandbox", action="store_true",
                               help="Get descriptor from Zenodo's sandbox instead of "
                               "production server.")
...
def add_subparser_import(subparsers):
    parser_import = subparsers.add_parser("import", description="Imports old descriptor or BIDS app or"
    ...
    parser_import.add_argument("-o", "--output-invocation", help="Where to write "
                               "the invocation if any.")
    parser_import.add_argument("-i", "--input-invocation", help="Input invocation "
                               " for CWL if any.")


def add_subparser_invocation(subparsers):
    parser_invocation = subparsers.add_parser("invocation", description="Creates invocation schema and"
    ...
    parser_invocation.add_argument("descriptor", action="store",
                                   help="The Boutiques descriptor as a JSON file, JSON "
                                   "string or Zenodo ID (prefixed by 'zenodo.').")
    parser_invocation.add_argument("-i", "--invocation", action="store",
                                   help="Input values in a JSON file or as a JSON "
                                   "object to be validated against "
                                   "the invocation schema.")
    parser_invocation.add_argument("-w", "--write-schema", action="store_true",
                                   help="If descriptor doesn't have an invocation "
                                   "schema, creates one and writes it to the descriptor"
                                   " file ")
    parser_invocation.add_argument("--sandbox", action="store_true",
                                   help="Get descriptor from Zenodo's sandbox instead of "
                                   "production server.")


def add_subparser_pprint(subparsers):
    parser_pprint = subparsers.add_parser("pprint", description="Boutiques pretty-print for"
    ...
    parser_pprint.add_argument("--sandbox", action="store_true",
                               help="Get descriptor from Zenodo's sandbox instead of "
                               "production server.")


def add_subparser_publish(subparsers):
    parser_publish = subparsers.add_parser("publish", description="A publisher of Boutiques tools"
    ...
    parser_publish.add_argument("--sandbox", action="store_true",
                                help="publish to Zenodo's sandbox instead of "
                                "production server. Recommended for tests.")
    parser_publish.add_argument("--zenodo-token", action="store",
                                help="Zenodo API token to use for authentication. "
                                "If not used, token will be read from configuration "
                                "file or requested interactively.")
    ...
    publish_mutex_group.add_argument("-r", "--replace", action="store_true",
    ...
    publish_mutex_group.add_argument("--id", action="store",
...
def add_subparser_pull(subparsers):
    parser_pull = subparsers.add_parser("pull", description="Ensures that Zenodo descriptors are"
...
def add_subparser_search(subparsers):
    parser_search = subparsers.add_parser("search", description="Search Zenodo for Boutiques"
...
def add_subparser_test(subparsers):
    parser_test = subparsers.add_parser("test", description="Perform all the tests defined within"
                                        " the given descriptor")
    parser_test.set_defaults(function='test')
    parser_test.add_argument("descriptor", action="store",
                             help="The Boutiques descriptor as a JSON file, JSON "
                             "string or Zenodo ID (prefixed by 'zenodo.').")
    parser_test.add_argument("--sandbox", action="store_true",
                             help="Get descriptor from Zenodo's sandbox instead of "
                             "production server.")
...
def add_subparser_validate(subparsers):
    ...
    parser_validate.add_argument("descriptor", action="store",
                                 help="The Boutiques descriptor as a JSON file, JSON "
                                 "string or Zenodo ID (prefixed by 'zenodo.').")
    parser_validate.add_argument("--bids", "-b", action="store_true",
                                 help="Flag indicating if descriptor is a BIDS app")
    parser_validate.add_argument("--format", "-f", action="store_true",
                                 help="If descriptor is valid, rewrite it with sorted"
                                 " keys.")
    parser_validate.add_argument("--sandbox", action="store_true",
                                 help="Get descriptor from Zenodo's sandbox instead of "
                                 "production server.")
