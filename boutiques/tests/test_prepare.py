...
class TestPrepare(BaseTest):
    ...
    def test_prepare_sing(self):
        ret = bosh.execute("prepare", self.get_file_path("example1_sing.json"))
    ...
    def test_prepare_sing_specify_imagepath(self):
        ret = bosh.execute(
            ...
            "--imagepath",
            os.path.join(os.getcwd(), "boutiques-example1-test.simg"),
        )
        self.assertIn("Local (boutiques-example1-test.simg)", ret.stdout)
    ...
    @mock.patch(
        "boutiques.localExec.LocalExecutor._cleanUpAfterSingPull",
    ...
    def test_prepare_sing_multiple_processes(
        self, mock_mkdir, mock_exists, mock_clean_up
    ):
    ...
    @mock.patch(
        "boutiques.localExec.LocalExecutor._localExecute",
