...
class Importer:
    ...
    def upgrade_04(self):
        ...
        if descriptor["schema-version"] != "0.4":
            raise_error(
                ImportError,
                "The input descriptor must have " "'schema-version'=0.4",
            )
    ...
    def import_cwl(self):
        ...
        if cwl_desc.get("baseCommand") is None:
            raise_error(
                ImportError,
                "Cannot find baseCommand attribute, "
        ...
        if cwl_desc.get("arguments"):
            for i in cwl_desc["arguments"]:
                ...
                if "$(runtime." in i:
                    raise_error(
                        ImportError,
                        "Runtime parameters " " are not supported:" " " + i,
                    )
                command_line += i + " "
        boutiques_inputs = []
        ...
        def position(x):
            if (
                type(x) is dict
                and x.get("inputBinding")
                and x["inputBinding"].get("position")
            ):
                return x["inputBinding"]["position"]
            return 0

        sorted_inputs = sorted(
            cwl_desc["inputs"], key=lambda x: (position(cwl_desc["inputs"][x]))
        )
        ...
        for cwl_input in sorted_inputs:
            ...
            if cwl_input_binding.get("prefix"):
                ...
                if (
                    not (cwl_input_binding.get("separate") is None)
                    and cwl_input_binding["separate"] is False
                ):
        ...
        sorted_outputs = sorted(
            cwl_desc["outputs"],
            key=(lambda x: cwl_desc["outputs"][x].get("outputBinding")),
        )
            ...
            if cwl_out_binding and cwl_out_binding.get("glob"):
                ...
                if type(cwl_out_obj.get("type")) is dict:
                    if (
                        ...
                        raise_error(
                            ImportError,
                            "Unsupported output type: " + cwl_output["type"],
                        )
            ...
            if req_type == "EnvVarRequirement":
                ...
                for env_var in req["envDef"]:
                    ...
                    bout_env_var["value"] = resolve_glob(
                        req["envDef"][env_var], boutiques_inputs
                    )
        ...
        # Write descriptor
        with open(self.output_descriptor, "w") as f:
            f.write(json.dumps(customSortDescriptorByKey(bout_desc), indent=4))
        validate_descriptor(loadJson(self.output_descriptor))
        ...
        with open(self.input_invocation) as f:
        ...
        with open(self.output_invocation, "w") as f:
            f.write(
                json.dumps(
                    customSortInvocationByInput(
                        boutiques_invocation, json.dumps(bout_desc)
                    ),
                    indent=4,
                )
            )
    ...
    def import_config(self):
        def _getConfigFileString():
            with open(self.input_descriptor) as configFile:
        ...
        def _getOutputConfigFileTemplate(configFileFormat):
            return {
                ...
                "path-template": f"config.{configFileFormat}",
                "file-template": [],
            }
...
class Docopt_Importer:
    ...
    def __init__(self, docopt_str, base_descriptor):
        ...
        with open(base_descriptor) as base_desc:
    ...
    def generateInputsAndCommandLine(self, node):
        ...
        if hasattr(node, "children") and (
            child_node_type == "Either" or child_node_type == "Required"
        ):
            ...
            self.descriptor["command-line"] = (
                self._parse_section("usage:", self.docopt_str)[0]
                .split("\n")[1:][0]
                .split()[0]
            )
    ...
    def _generateDependencyArgument(
        self, node, isList=False, optional=False, members=[]
    ):
        ...
        new_arg = {
            ...
            "desc": (
                self.all_desc_and_type[node.name]["desc"]
    ...
    def _addGroupArgumentToDependencies(self, arg, ancestors, optional=False):
        ...
        self._addArgumentToDependencies(
            grp_arg, ancestors=ancestors, optional=optional, members=members
        )
    ...
    def addInputsRecursive(self, node, requires=[]):
        ...
        if len(args_ids) == 1:
            ...
            self.addInputsRecursive(node[args_ids[0]]["children"], requires)
        elif len(args_ids) > 1:
            mutex_names = []
    ...
    def _addInputOrMutexGroupToDescriptor(
        self, node, requires, addInputWithLineage=False
    ):
        ...
        for ancestor in ancestors:
            if last_node is None:
                ...
                last_node = last_node["children"][
                    [
                        {"id": last_node["children"][n]["id"], "key": n}
                        for n in last_node["children"]
                    ][-1]["key"]
                ]
            else:
                return None
