...
def main(args=None):
    ...
    parser.add_argument(
        ...
        nargs=1,
        help='A string in {"val1", "val2", "val3"}.',
    )
    ...
    if homeDir != os.getenv("PWD"):
        sys.stderr.write(
            "error: HOME environment variable must be set to the current"
            " working directory.\n"
        )
        sys.exit(1)
    ...
    sys.stdout.write("�a c'est stdout")
    sys.stderr.write("This is std�rr")
