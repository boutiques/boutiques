...
def validate_descriptor(descriptor, **kwargs):
    ...
    if "output-files" in descriptor:
        ...
        # or is 'default'
        msg_template = (
            'OutputError: "{0}" contains non-python keyword and '
            'non-ID string: "{1}"'
        )
    ...
    for inp in descriptor["inputs"]:
        ...
        # Verify number-type inputs min/max are sensible
            ...
            msg_template = (
                ' InputError: "{0}" cannot have greater'
                " min ({1}) than max ({2})"
            )
        ...
        # Verify enum-type inputs (at least 1 option, default in set)
            ...
            msg_template = (
                ' InputError: "{0}" must have at least' " one value choice"
            )
            ...
            msg_template = (
                ' InputError: "{0}" cannot have default'
        ...
        if "value-choices" in inp.keys():
            ...
            if (
                "value-requires" in inp.keys()
                and "value-disables" in inp.keys()
            ):
                msg_template = (
                    ' InputError: "{0}" choice "{1}" requires'
                    ' and disables "{2}"'
                )
                errors += [
    ...
    if "tests" in descriptor.keys():
        ...
        for test in descriptor["tests"]:
            ...
            if "output-files" in test["assertions"].keys():
                test_output_ids = safeGet(
                    test["assertions"], "output-files", "id"
                )

                # Verify if output reference ids are valid
                msg_template = (
                    'TestError: "{0}" output id' ' not found, in test "{1}"'
                )
                errors += [
