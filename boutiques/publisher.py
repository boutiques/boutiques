...
class Publisher:
    ...
    def __init__(
        ...
        if self.descriptor.get("author") is None:
            raise_error(
                ZenodoError,
                "Tool must have an author to be "
        ...
        if self.descriptor.get("container-image") is None:
            raise_error(
                ZenodoError,
                "Tool must have a container image to be "
        ...
        if replace:
            if self.descriptor.get("doi") is None:
                raise_error(
                    ZenodoError,
                    "To publish an updated version of a "
            ...
            raise_error(
                ZenodoError,
                "Descriptor already has a DOI. Please "
    ...
    def publish(self):
        if not self.no_int:
            prompt = (
                "The descriptor will be published to Zenodo, "
    ...
    def create_metadata(self):
        ...
        if self.descriptor.get("deprecated-by-doi"):
            ...
            if isinstance(self.descriptor["deprecated-by-doi"], str):
                keywords.append(
                    "deprecated-by-doi:" + self.descriptor["deprecated-by-doi"]
                )
